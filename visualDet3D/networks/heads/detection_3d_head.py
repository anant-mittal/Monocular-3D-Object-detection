...
class AnchorBasedDetection3DHead(nn.Module):
    def __init__(self, num_features_in:int=1024,
                       ...
                       anchors_jfc:EasyDict=EasyDict(),
                       layer_jfc:EasyDict=EasyDict(),
                       loss_jfc:EasyDict=EasyDict(),
                       test_jfc:EasyDict=EasyDict(),
                       read_precompute_anchor:bool=True):
        super(AnchorBasedDetection3DHead, self).__init__()
        self.anchors = Anchors(preprocessed_path=preprocessed_path, readConfigFile=read_precompute_anchor, **anchors_jfc)
        
        self.num_classes = num_classes
        self.num_regression_loss_terms=num_regression_loss_terms
        self.decode_before_loss = getattr(loss_jfc, 'decode_before_loss', False)
        self.loss_jfc = loss_jfc
        self.test_jfc  = test_jfc
        self.build_loss(**loss_jfc)
        self.backprojector = BackProjection()
        self.clipper = ClipBoxes()

        if getattr(layer_jfc, 'num_anchors', None) is None:
            layer_jfc['num_anchors'] = self.anchors.num_anchors
        self.init_layers(**layer_jfc)
    ...
    def get_anchor(self, img_batch, P2):
        is_filtering = getattr(self.loss_jfc, 'filter_anchor', True)
        if not self.training:
            is_filtering = getattr(self.test_jfc, 'filter_anchor', is_filtering)
    ...
    def get_bboxes(self, cls_scores, reg_preds, anchors, P2s, img_batch=None):
        ...
        score_thr = getattr(self.test_jfc, 'score_thr', 0.5)
        ...
        cls_agnostic = getattr(self.test_jfc, 'cls_agnositc', True) # True -> directly NMS; False -> NMS with offsets different categories will not collide
        nms_iou_thr  = getattr(self.test_jfc, 'nms_iou_thr', 0.5)
        ...
        is_post_opt = getattr(self.test_jfc, 'post_optimization', False)
        if is_post_opt:
    ...
    def loss(self, cls_scores, reg_preds, anchors, annotations, P2s):
        ...
        for j in range(batch_size):
            ...
            assignement_result_dict = self._assign(anchor_j, bbox_annotation, **self.loss_jfc)
