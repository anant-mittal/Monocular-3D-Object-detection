...
@torch.no_grad()
def evaluate_kitti_depth(jfc:EasyDict, 
                       model:nn.Module,
    ...
    model.eval()
    result_path = os.path.join(jfc.path.preprocessed_path, result_path_split, 'data')
    ...
    if "is_running_test_set" in jfc and jfc["is_running_test_set"]:
        print("Finish evaluation.")
        return
    result_texts = evaluate_depth(
        label_path = os.path.join(jfc.path.validation_path, 'groundtruth_depth'),
...
@PIPELINE_DICT.register_module
@torch.no_grad()
def evaluate_kitti_obj(jfc:EasyDict, 
                       model:nn.Module,
                       dataset_val:Sized,
                       writer:SummaryWriter,
                       epoch_num:int,
    ...
    os.mkdir(result_path)
    print("rebuild {}".format(result_path))
    testing_function = PIPELINE_DICT[jfc.trainer.testing_function]
    projector = BBox3dProjector().cuda()
    backprojector = BackProjection().cuda()
    for index in tqdm(range(len(dataset_val))):
        test_one(jfc, index, dataset_val, model, testing_function, backprojector, projector, result_path)
    if "is_running_test_set" in jfc and jfc["is_running_test_set"]:
        print("Finish evaluation.")
        return
    result_texts = evaluate(
        label_path=os.path.join(jfc.path.data_path, 'label_2'),
        result_path=result_path,
        label_split_file=jfc.data.val_split_file,
        current_classes=[i for i in range(len(jfc.obj_types))],
...
def test_one(jfc, index, dataset, model, testing_function, backprojector:BackProjection, projector:BBox3dProjector, result_path):
    data = dataset[index]
    ...
    scores, bbox, obj_names = testing_function(collated_data, model, None, jfc=jfc)
    bbox_2d = bbox[:, 0:4]
    if bbox.shape[1] > 4: # run 3D
        ...
        if "crop_top" in jfc.data.augmentation and jfc.data.augmentation.crop_top is not None:
            crop_top = jfc.data.augmentation.crop_top
        elif "crop_top_height" in jfc.data.augmentation and jfc.data.augmentation.crop_top_height is not None:
            if jfc.data.augmentation.crop_top_height >= original_height:
                crop_top = 0
            else:
                crop_top = original_height - jfc.data.augmentation.crop_top_height

        scale_2d = (original_height - crop_top) / height
        bbox_2d[:, 0:4] *= scale_2d
        bbox_2d[:, 1:4:2] += jfc.data.augmentation.crop_top
