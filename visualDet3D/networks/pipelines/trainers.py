...
def train_mono_detection(data, module:nn.Module,
    ...
    if max_length == 0:
        return
    annotation = compound_annotation(labels, max_length, bbox2d, bbox_3d, jfc.obj_types) #np.arraym, [batch, max_length, 4 + 1 + 7]
    ...
    loss.backward()
    # clip loss norm
    torch.nn.utils.clip_grad_norm_(module.parameters(), jfc.optimizer.clipped_gradient_norm)
...
def train_rtm3d(data, module:nn.Module,
    ...
    loss.mean().backward()
    # clip loss norm
    if 'clipped_gradient_norm' in jfc.optimizer:
        torch.nn.utils.clip_grad_norm_(module.parameters(), jfc.optimizer.clipped_gradient_norm)
    optimizer.step()
