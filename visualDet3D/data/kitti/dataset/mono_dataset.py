...
class KittiMonoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, jfc, split='training'):
        super(KittiMonoDataset, self).__init__()
        preprocessed_path   = jfc.path.preprocessed_path
        obj_types           = jfc.obj_types
        ...
        if is_train:
            self.transform = build_augmentator(jfc.data.train_augmentation)
        else:
            self.transform = build_augmentator(jfc.data.test_augmentation)
        self.projector = BBox3dProjector()
        self.is_train = is_train
        self.obj_types = obj_types
        self.use_right_image = getattr(jfc.data, 'use_right_image', True)
        self.is_reproject = getattr(jfc.data, 'is_reproject', True) # if reproject 2d
...
class KittiMonoTestDataset(KittiMonoDataset):
    def __init__(self, jfc, split='test'):
        preprocessed_path   = jfc.path.preprocessed_path
        obj_types           = jfc.obj_types
        super(KittiMonoTestDataset, self).__init__(jfc, 'test')
        is_train = (split == 'training')
