...
class KittiDepthMonoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, jfc, split='training'):
        ...
        raw_path    = jfc.path.raw_path
        depth_paths  = jfc.path.depth_path if isinstance(jfc.path.depth_path, list) else [jfc.path.depth_path]
        aug_jfc     = jfc.data.augmentation
        manager = Manager() # multithread manage wrapping for list objects
        self.is_eval = not split == 'training'
        self.size = aug_jfc.cropSize #[352, 1216]
        raw_sequences = {}
...
class KittiDepthMonoValTestDataset(torch.utils.data.Dataset):
    """Some Information about KittiDataset"""
    def __init__(self, jfc, split='validation'):
        super(KittiDepthMonoValTestDataset, self).__init__()
        base_path = jfc.path[split + "_path"]
        self.transform = build_augmentator(jfc.data.test_augmentation)
