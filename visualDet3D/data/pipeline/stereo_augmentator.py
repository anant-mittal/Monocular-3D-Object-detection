...
class Augmentation(object):
    ...
    def __init__(self, jfc):

        self.mean = jfc.rgb_mean
        self.stds = jfc.rgb_std
        self.size = jfc.cropSize
        self.mirror_prob = jfc.mirrorProb
        self.distort_prob = jfc.distortProb

        if jfc.distortProb <= 0:
            self.augment = Compose.from_transforms([
                ConvertToFloat(),
                CropTop(jfc.crop_top),
                Resize(self.size),
...
class Shuffle(object):
    ...
    def __init__(self, aug_list:List[EasyDict]):
        self.transforms = [
            build_single_augmentator(aug_jfc) for aug_jfc in aug_list
        ]
