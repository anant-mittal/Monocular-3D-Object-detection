...
def precompute_depth_statistic(config:str="config/config.py", gpu=0):
    ...
    dataset = DATASET_DICT[dataset_name](jfc, 'val') # being eval
    torch.cuda.set_device(gpu)

    save_dir = os.path.join(jfc.path.preprocessed_path, 'training')
    if not os.path.isdir(save_dir):
        os.makedirs(save_dir)

    sum_data = torch.zeros([jfc.data.rgb_shape[0], jfc.data.rgb_shape[1]]).cuda() #[H,W]
    number_solid = torch.zeros_like(sum_data, dtype=torch.long).cuda()
    dataloader_train = DataLoader(dataset, num_workers=jfc.data.num_workers,
            batch_size=jfc.data.batch_size, collate_fn=dataset.collate_fn, shuffle=False, drop_last=False)
