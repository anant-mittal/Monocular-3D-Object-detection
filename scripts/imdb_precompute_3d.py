...
def process_train_val_file(jfc):
    train_file = jfc.data.train_split_file
    val_file   = jfc.data.val_split_file
...
def read_one_split(jfc, index_names, data_root_dir, output_dict, data_split = 'training', time_display_inter=100):
    save_dir = os.path.join(jfc.path.preprocessed_path, data_split)
    ...
    anchor_prior = getattr(jfc, 'anchor_prior', True)

    total_objects = [0 for _ in range(len(jfc.obj_types))]
    total_usable_objects = [0 for _ in range(len(jfc.obj_types))]
    if anchor_prior:
        anchor_manager = Anchors(jfc.path.preprocessed_path, readConfigFile=False, **jfc.detector.head.anchors_jfc)
        preprocess = build_augmentator(jfc.data.test_augmentation)
        total_objects = [0 for _ in range(len(jfc.obj_types))]
        total_usable_objects = [0 for _ in range(len(jfc.obj_types))]
        ...
        examine = np.zeros([len(jfc.obj_types), len_level * len_scale, len_ratios])
        sums    = np.zeros([len(jfc.obj_types), len_level * len_scale, len_ratios, 3]) 
        squared = np.zeros([len(jfc.obj_types), len_level * len_scale, len_ratios, 3], dtype=np.float64)

        uniform_sum_each_type = np.zeros((len(jfc.obj_types), 6), dtype=np.float64) #[z, sin2a, cos2a, w, h, l]
        uniform_square_each_type = np.zeros((len(jfc.obj_types), 6), dtype=np.float64)
    ...
    for i, index_name in enumerate(index_names):
        ...
        max_occlusion = getattr(jfc.data, 'max_occlusion', 2)
        min_z        = getattr(jfc.data, 'min_z', 3)
        if data_split == 'training':
            data_frame.label = [obj for obj in label.data if obj.type in jfc.obj_types and obj.occluded < max_occlusion and obj.z > min_z]
            
            if anchor_prior:
                for j in range(len(jfc.obj_types)):
                    total_objects[j] += len([obj for obj in data_frame.label if obj.type==jfc.obj_types[j]])
                    data = np.array(
                        [
                            [obj.z, np.sin(2 * obj.alpha), np.cos(2 * obj.alpha), obj.w, obj.h, obj.l] 
                                for obj in data_frame.label if obj.type==jfc.obj_types[j]
                        ]
                    ) #[N, 6]
                    if data.any():
            ...
            data_frame.label = [obj for obj in label.data if obj.type in jfc.obj_types]
        ...
        if data_split == 'training' and anchor_prior:
            ...
            if len(data_frame.label) > 0:
                ...
                for j in range(len(jfc.obj_types)):
                    bbox2d = torch.tensor([[obj.bbox_l, obj.bbox_t, obj.bbox_r, obj.bbox_b] for obj in label if obj.type == jfc.obj_types[j]]).cuda()
                    if len(bbox2d) < 1:
                        continue
                    bbox3d = torch.tensor([[obj.x, obj.y, obj.z, np.sin(2 * obj.alpha), np.cos(2 * obj.alpha)] for obj in label if obj.type == jfc.obj_types[j]]).cuda()
                    ...
                    num_usable_object = torch.sum(IoU_max > jfc.detector.head.loss_jfc.fg_iou_threshold).item()
                    total_usable_objects[j] += num_usable_object

                    positive_anchors_mask = IoU_max_anchor > jfc.detector.head.loss_jfc.fg_iou_threshold
    ...
    if data_split == 'training' and anchor_prior:
        ...
        for j in range(len(jfc.obj_types)):
            ...
            npy_file = os.path.join(save_dir,'anchor_mean_{}.npy'.format(jfc.obj_types[j]))
            np.save(npy_file, avg)
            std_file = os.path.join(save_dir,'anchor_std_{}.npy'.format(jfc.obj_types[j]))
...
def main(config:str="config/config.py"):
    ...
    data_root_dir = jfc.path.data_path # the base directory of training dataset
    ...
    train_names, val_names = process_train_val_file(jfc)
    read_one_split(jfc, train_names, data_root_dir, output_dict, 'training', time_display_inter)
    output_dict = {
                "calib": True,
                "image": False,
                "label": True,
    ...
    read_one_split(jfc, val_names, data_root_dir, output_dict, 'validation', time_display_inter)
