...
def process_train_val_file(jfc)-> Tuple[List[str], List[str]]:
...
def compute_dispairity_for_split(jfc,
    ...
    for i, index_name in tqdm(enumerate(index_names)):
        ...
        if use_point_cloud:
            ...
            left_image  = denorm(image, jfc.data.augmentation.rgb_mean, jfc.data.augmentation.rgb_std)
            right_image = denorm(image_3, jfc.data.augmentation.rgb_mean, jfc.data.augmentation.rgb_std)
...
def main(config:str="config/config.py",use_point_cloud:bool=False):
    ...
    compute_dispairity_for_split(jfc, train_names, data_root_dir, output_dict, 'training', time_display_inter, use_point_cloud)
